...
df=fread('path/to/indivdual/lungcancer/data')
prscxs=fread('PRS/LungCancer/compiled_PRSCSX.csv')
head(prscxs)
...
for(smoke in c('all','nonsmoker','smoker')){
  for(anc in c('all','afr','eur')){
    ...
    if(anc=='all'){
      prscol=which(colnames(dfanc)==paste('LungCancer_eur_prscs',sep='')) 
    }
    ...
    write.csv(em,paste('LC_analysis/modelweights_',smoke,'_',anc,'.csv',sep=''))
    ...
    write.csv(dftest,paste('LungCancer_',smoke,'_',anc,'.csv',sep=''))
